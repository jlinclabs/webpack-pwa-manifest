        ...
        for (var _iterator2 = assets[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
          ...
          if (asset.ios && asset.ios.valid) {
            if (asset.ios.valid === 'startup') {
              var _asset$ios$size$match = asset.ios.size.match(/^(\d+)x(\d+)$/).slice(1, 3).map(function (x) {
                return parseInt(x, 10);
              }),
                  _asset$ios$size$match2 = _slicedToArray(_asset$ios$size$match, 2),
                  width = _asset$ios$size$match2[0],
                  height = _asset$ios$size$match2[1];

              var pixelRatio = asset.ios.pixelRatio || 2;
              var orientation = width < height ? 'portrait' : 'landscape';
              var media = '(device-width: ' + Math.floor(width / pixelRatio) + 'px)' + (' and (device-height: ' + Math.floor(height / pixelRatio) + 'px)') + (' and (-webkit-device-pixel-ratio: ' + pixelRatio + ')') + (' and (orientation: ' + orientation + ')');
              applyTag(tags, 'link', {
